...
void addAdmin()
{
    ...
    if(fp==NULL)
    {
        ...
        user u[2]={{"admin","abc","ramesh"},{"suresh","abc","suresh"}};
...
void bookedCarDetails()
{
    ...
    int x=10;
    textcolor(YELLOW);
    while(fread(&cc,sizeof(customer_car_detail),1,fp)==1)
    {
        gotoxy(1,x);
...
int rentCar()
{
    ...
    *pos='\0';
    ...
    do
    {
        ...
        fwrite(&cc,sizeof(customer_car_detail),1,fp);
...
void availableCarDetails()
{
    ...
    for(i=1;i<80;i++)
        printf("%c",247);
    gotoxy(1,8);
    printf(" Car ID\t\tName\t\tCapacity\t\tAvailable\t\tPrice/Day");
    gotoxy(1,9);
    textcolor(LIGHTGREEN);
    for(i=1;i<80;i++)
        printf("%c",247);
